...
export function evaluateExtended(entity,rule,snap){
  ...
  if(['ca253','ca261'].includes(rule.kind)){
    ...
    trace=[fact('Business formed under US law',entity.us_formed),fact('Doing business in California (explicit illustrative nexus)',entity.ca_business),over(`FY 2025 total annual revenue > USD ${amount}m`,entity.ca_revenue_2025_musd,amount)];
    ...
    const gates=[fact('EU undertaking (illustrative France entity)',entity.eu_undertaking),over('Average employees > 1,000 (illustrative selected-year input)',entity.eu_employees,1000),over('Annual net turnover > EUR 450m (illustrative selected-year input)',entity.eu_turnover_meur,450)];
...
export function supplierRows(state,snap,isMember){
  ...
  const evidence=mode==='primary'?'Illustrative supplier sample available; allocation, boundary and assurance unverified':mode==='secondary'?'Illustrative secondary-data method proposed; no emissions number calculated':'No activity or emissions data provided';
...
export const TOPIC_TEXT={
  ...
  california:'Atlas Americas has illustrative FY 2025 revenue of USD 1,500m and California business activity; California Distribution has USD 750m. Their statutory screens therefore differ. The results below separate scope from SB 253 implementation uncertainty and the SB 261 injunction. A qualifying parent report may cover a subsidiary, but inclusion and equivalence must be demonstrated. This is the fixed 2026 initial cycle, not a rolling deadline calendar.',
