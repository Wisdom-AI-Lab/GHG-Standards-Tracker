...
const developmentNav=active=>`<nav class="section-nav" aria-label="Developments sections"><a href="#updates" ${active==='updates'?'aria-current="page"':''}>Development log</a><a href="#compare" ${active==='compare'?'aria-current="page"':''}>Compare regulatory impacts</a></nav>`;
...
function method(data,today){const stats=summarise(data.records,today);return `<div class="method-grid"><div><section class="method-section"><p class="eyebrow">RESEARCH STANDARD</p><h2>What each evidence label means</h2><p><strong>Source checked:</strong> an official source was read to support the bounded summary and dates. It is not a full standards audit or a human sign-off.</p><p><strong>Human reviewed:</strong> a named reviewer and review date are recorded. This still does not establish applicability to a particular entity.</p><p><strong>Not verified:</strong> supporting evidence is incomplete. Treat the record as a research lead.</p><p>Checks older than 30 days are flagged for rechecking. The clock does not automatically change the legal or technical status of a record.</p></section><section class="method-section"><p class="eyebrow">PROPOSED OPERATING MODEL</p><h2>Research → verify → approve</h2><ol><li>Research approved primary sources; save dated evidence and a proposed change.</li><li>Independently verify the claim, date type and source. Flag conflicting information.</li><li>Validate the dataset and submit a pull request with the evidence.</li><li>A human approves material changes before publication.</li><li>Periodically recheck existing records, not just new announcements.</li></ol><p><strong>Monitoring status:</strong> no model provider, API credentials, search service or scheduled agent has been configured. Near real-time monitoring is a planned capability. The GitHub check currently validates files; it does not research regulations.</p></section></div><div><section class="method-section"><p class="eyebrow">CURRENT EDITION</p><h2>${e(data.edition||'Research starter')}</h2><div class="queue-row"><strong>Edition date</strong><span>${e(formatDate(data.prepared_on))}</span></div><div class="queue-row"><strong>Human review pending</strong><span>${stats.pending} records</span></div><div class="queue-row"><strong>Source rechecks due</strong><span>${stats.stale} records</span></div><p style="margin-top:15px">The initial summaries were written from official sources. They are a selected starting set, not comprehensive coverage of all GHG standards or legal developments.</p></section><section class="method-section"><p class="eyebrow">COVERAGE TO EXPAND</p><h2>Research backlog</h2><p>CSRD / ESRS revisions, broader jurisdictional ISSB adoption and sector-specific target standards remain research priorities. TNFD and selected California records are included; their scope and implementation limits remain explicit.</p><p>Absence from this tracker never means an obligation does not apply.</p></section><section class="method-section"><h2>Data and implementation provenance</h2><p>This working version uses new code, original styling and a new dataset. The earlier tracker was a functional reference only. No original author endpoint, images or compiled dataset is used by this version. The client workspace separately labels a small public PepsiCo subsidiary extract and illustrative Atlas scenarios.</p><p><strong>Illustrative company data:</strong> Atlas entities, financial inputs, materiality assessments and transactions are sample scenarios, not verified client records. PepsiCo is a separate, limited public-data extract.</p><p>The GitHub repository remains a fork and retains its earlier history. This project does not claim ownership of that history or grant rights to third-party standards.</p></section></div></div><section class="method-section glossary"><p class="eyebrow">REFERENCE</p><h2>Glossary</h2><div class="table-wrap"><table><caption class="sr-only">ESG reporting terms</caption><thead><tr><th scope="col">Term</th><th scope="col">Meaning</th></tr></thead><tbody>${GLOSSARY.map(item=>`<tr><th scope="row">${e(item.term)}</th><td>${e(item.definition)}</td></tr>`).join('')}</tbody></table></div></section>`;}
...
export async function start(){
  ...
  function render(){
    ...
    document.querySelectorAll('.navigation nav a').forEach(a=>{if(a.getAttribute('href')==='#'+(view==='compare'?'updates':view))a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
